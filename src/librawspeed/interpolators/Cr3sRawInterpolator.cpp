...
#include "interpolators/Cr3sRawInterpolator.h"
...
struct Cr3sRawInterpolator::YCbCr final {
  int Y = 0;
  int Cb = 0;
  int Cr = 0;
...
template <int version> void Cr3sRawInterpolator::interpolate_422_row(int row) {
...
template <int version> void Cr3sRawInterpolator::interpolate_420() {
  const Array2DRef<uint16_t> out(mRaw->getU16DataAsUncroppedArray2DRef());
...
inline void Cr3sRawInterpolator::YUV_TO_RGB<0>(const YCbCr& p, uint16_t* X) {
  assert(X);
...
template </* int version */>
inline void Cr3sRawInterpolator::YUV_TO_RGB<1>(const YCbCr& p, uint16_t* X) {
  assert(X);
...
inline void Cr3sRawInterpolator::YUV_TO_RGB<2>(const YCbCr& p, uint16_t* X) {
  assert(X);
...
void Cr3sRawInterpolator::interpolate(int version) {
